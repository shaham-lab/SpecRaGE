...
def get_normalized_laplacian(W: torch.Tensor) -> np.ndarray:
    """
    ...
    L = np.identity(W.shape[0]) - np.dot(np.dot(D_inv_sqrt, W), D_inv_sqrt)
...
def plot_sorted_laplacian(W: torch.Tensor, y: np.ndarray, i):
    """
    ...
    plt.savefig(f"laplacian{i}.png")
    # plt.show()


def plot_diagonalization(W: torch.Tensor, Y: torch.Tensor):
    L = get_laplacian(W)
    diag = Y.T @ L @ Y
    diag /= 100
    # diag = diag @ Y
    cmap = plt.cm.hot_r
    print(diag)
    plt.imshow(diag, cmap=cmap)
    cbar = plt.colorbar()
    cbar.set_label("Diagonalization Values")
    plt.show()
...
def get_affinity_matrix(X: torch.Tensor) -> torch.Tensor:
    ...
    W = get_gaussian_kernel(
        Dx, scale, indices, device=torch.device("cpu"), is_local=is_local
    )
    return W
